...
pm25_tot_man <- read_csv("./data/pm2.5_tot_nyc.csv") %>% 
...
all_elements_man <- read_csv("./data/manhattan_daily_pm2.5_components.csv") %>% 
  janitor::clean_names() %>% 
...
conc <- all_elements_man %>% 
  select(-antimony, -cobalt) %>% 
  left_join(pm25_tot_man, by = "date") %>% 
  relocate(date, pm25)
...
ggcorr(all_elements_bx, method = c("pairwise.complete.obs", "pearson"), limits = FALSE,
       ...
       hjust = 1, size = 3, color = "grey50", layout.exp = 4) + ggtitle("Correlation Matrix: Manhattan Air Pollution Dataset")
...
vals <- c(74.8, 3.5, 0.62, 6.99, 11.1, 2.97)
val_names <- c("1 (74.8%)", "2 (3.5%)", "3 (0.62%)", "4 (6.99%)", "5 (11.1%)", "6 (2.97%)")
