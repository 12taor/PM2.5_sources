...
pm25_tot_avg <- read_csv("./data/pm2.5_total_components_daily_nyc_avg.csv") %>% 
  rename(
    date = date_local,
    pm25 = pm2.5_total
  ) %>% 
...
pm25_tot_raw <- read_csv("./data/pm2.5_nyc_master.csv") %>% 
  filter(
    site_num %in% c(110, 62, 134, 124),
    date_local > "2006-12-31"
    )
...
missingness_poc_bx <- pm25_tot_raw %>% 
  filter(
    site_num == 110,
    date_local < "2010-06-12"
    ) %>% 
  select(poc, date_local, arithmetic_mean) %>% 
  pivot_wider(
    names_from = poc,
    values_from = arithmetic_mean
    ) %>% 
  mutate(
    pmtot = ifelse(is.na(`1`) == FALSE, `1`, `2`)
  )

missingness_poc_bx %>% view()

is.na(missingness_poc_bx$`1`) %>% sum()

pm25_tot <- pm25_tot_raw %>% 
  pivot_wider(
    names_from = poc,
    values_from = arithmetic_mean
  ) %>% 
  mutate(
    pm_tot = ifelse(is.na(`1`) == FALSE, `1`, `2`),
    site = case_when(
      site_num == 110 ~ "bronx",
      site_num %in% c(62, 134) ~ "manhattan",
      site_num == 124 ~ "queens"
    )
  ) %>% 
  select(site_num, date_local, pm_tot, site)
...
pm25_tot <- read_csv("./data/pm2.5_tot_nyc.csv")
