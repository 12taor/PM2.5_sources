...
pacman::p_load(tidyverse, lubridate, ggmap, ggcorrplot, gplots, GGally, knitr, patchwork, psych, factoextra, RColorBrewer)
...
conc <- read_csv("./data/nyc_daily_pm2.5_components.csv") %>% 
  janitor::clean_names() %>% 
...
K_year <- conc %>% 
  mutate(year = format(date, "%Y"),
         CommonDate = as.Date(paste0("2004-", format(date, "%j")), "%Y-%j")) %>%
  ggplot(aes(x = CommonDate, y = potassium_ion)) +
  geom_line() +
...
conc %>% 
  select(date, potassium_ion) %>% 
  filter(potassium_ion > 0.2) %>% 
  kable(caption = "dates  where Potassium > 200")

conc %>% 
  select(date, potassium_ion) %>% 
  filter(potassium_ion > 0.15) %>% 
  kable(caption = "dates  where Potassium > 150")
...
SA_Varimax <- mean_source %>% 
  ...
  mutate(
    PredConc = round(reduce(select(., contains("source")), `+`), digits = 2),
