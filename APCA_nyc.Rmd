...
pacman::p_load(tidyverse, lubridate, ggmap, ggcorrplot, gplots, GGally, knitr, patchwork, psych, factoextra, RColorBrewer, kableExtra)
...
conc %>% 
  ...
  filter(potassium_ion > 200) %>% 
  kable(caption = "dates  where Potassium > 200 ng/m3")

conc %>% 
  select(date, potassium_ion) %>% 
  filter(potassium_ion > 150) %>% 
