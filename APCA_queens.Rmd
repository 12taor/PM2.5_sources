...
pm25_tot_queens <- read_csv("./data/pm2.5_tot_nyc.csv") %>% 
  filter(site == "queens") %>% 
...
all_elements_queens <- read_csv("./data/queens_daily_pm2.5_components.csv") %>% 
  janitor::clean_names() %>% 
...
conc <- all_elements_queens %>% 
  select(-antimony, -cobalt) %>% 
  left_join(pm25_tot_queens, by = "date") %>% 
  relocate(date, pm25)
...
ggcorr(all_elements_queens, method = c("pairwise.complete.obs", "pearson"), limits = FALSE,
       label = TRUE, label_size = 1.5, label_alpha = TRUE,
       hjust = 1, size = 3, color = "grey50", layout.exp = 4) + ggtitle("Correlation Matrix: Queens Air Pollution Dataset")
...
vals <- c(31.6, 3.37, 14.0, 5.21, 0.85, 44.9)
val_names <- c("1 (31.6%)", "2 (3.37%)", "3 (14.0%)", "4 (5.21%)", "5 (0.85%)", "6 (44.9%)")
