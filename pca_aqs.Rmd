...
Create list of datasets and scale for PCA

```{r}

list_components =
  list(
    all = components_all %>% select(-X1, -date_local),
    any = components_any %>% select(-date_local),
    queens = components_124 %>% select(-X1, -date_local)
  )
...
scale_pollutants = function(df){
  data = df %>% 
    scale(center = FALSE, scale = apply(data = df, 2, sd, na.rm = TRUE))
...
#  mutate(
#    dataset = 
#      map(.x = list_components_df$dataset, ~ scale(.x, center = FALSE, scale = apply(.x, 2, sd, na.rm = TRUE)))
#  )
...
list_components_df =
  list_components_df %>% 
  mutate(
    PCA = map(list_components$dataset, prcomp)
  )

PCA_list <- map(list_components$dataset, prcomp)
